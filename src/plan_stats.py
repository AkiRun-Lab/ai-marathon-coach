...
_MENU_KEYWORD_MAP = [
    ...
    (["レペ"], "R"),
    (["マラソンペース", "Mペース"], "M"),
    (["ジョグ", "イージー", "回復", "ロング", "LSD", "Eペース", "ラン"], "E"),
]

# 文字列中の強度記号（E/M/T/I/R）を検出する正規表現
# 例: "E 4:50〜5:00/km" / "R 3:12/km（200m≒38秒）" / "T20分" / "E＋流し" /
#     "Eランニング" / "Tクルーズインターバル" / 複合練習 "E 5:33〜4:55/km, M 4:30/km" 等
#     （先頭に限らずすべて拾う。記号直後はスペース・数字・記号のほか、かな・カナ・漢字も許容）
# 前後に英字が続く場合（"Easy"のE等、英単語の一部）は記号ではないため除外
_PACE_SYMBOL_RE = re.compile(r"(?<![A-Za-z])(E|M|T|I|R)(?=[\s:：0-9+＋/／ぁ-ゖァ-ヶ一-龯])")
