...
| 定数名 | デフォルト値 | 説明 |
|:-------|:-------------|:-----|
| GEMINI_DEFAULT_MODEL | gemini-3.7-flash | 使用するGeminiモデル |
| GEMINI_FALLBACK_MODEL | gemini-3.6-flash | 503（高負荷）時のフォールバックモデル |
...
### 使用モデル
**gemini-3.7-flash** - 高度な推論能力を持つ最新モデル（503高負荷時は gemini-3.6-flash にフォールバック）
