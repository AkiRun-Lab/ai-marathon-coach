...
8. **練習レース前日・前々日はポイント練習禁止**（Eペースジョグまたは休養のみ）
9. 練習レース翌日はリカバリージョグ

## 【最重要】ポイント練習（Q）のルール
10. **ポイント練習は週{user_data.get('point_training_days', '2')}回**とすること（練習レースを含む）
11. **ポイント練習を2日連続で行ってはいけない**（必ず間に1日以上の回復日を入れる）
12. ポイント練習の種類：インターバル走、テンポ走、ペース走、ロング走（25km以上）、練習レース
13. **ロング走（25km以上）はポイント練習（Q）としてカウント**すること
14. 練習レースがある週は、練習レースの前後に回復日を確保しつつ、週のポイント練習回数を調整すること

## トレーニング内容ルール
15. 各フェーズで、上記で指定したそのフェーズのVDOTに対応したペースを必ず使用すること
16. **フェーズ1は現在のVDOT（{current_vdot}）**で練習すること
17. 週間走行距離は{user_data.get('weekly_distance', '不明')}kmを目安にすること
18. トレーニング開始日は{start_date_str}（月曜日）から始めること
19. 全{training_weeks}週間分のメニューを出力すること
20. 最終週はレースウィークとしてテーパリングを入れ、レース当日まで7日間出力すること
21. レース日は必ず{race_date_with_day}とすること（この日付を最終日として逆算してスケジュールを組むこと）
"""
