...
def convert_json_to_markdown(json_str: str, user_data: dict = None) -> Tuple[Optional[str], int]:
    ...
    Returns:
        (Markdown文字列, 週数) のタプル。週数は weekly_schedules の実要素数で、
        呼び出し側の「全週出力されたか」チェックに使う（JSONの二重パース回避）。
        パース・変換に失敗した場合は (None, 0)
